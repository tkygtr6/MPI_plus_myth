...
void *send_func(void* args_){
    send_type *args = (send_type *) args_;
    MPI_Send(args->buf, args->count, args->datatype, args->dest, args->tag, args->comm);
}

void *recv_func(void* args_){
    recv_type *args = (recv_type *) args_;
    MPI_Recv(args->buf, args->count, args->datatype, args->source, args->tag, args->comm, args->status);
}

myth_thread_t threads[MAX_THREAD];
send_type s[MAX_THREAD];
recv_type r[MAX_THREAD];
                   ...
                   VectorType& x)
{
  ...
  for (int i = 0; i < num_neighbors; i++)
    {
      ...
      r[i].buf = x_external;
      r[i].count = n_recv;
      r[i].datatype = mpi_dtype;
      r[i].source = neighbors[i];
      r[i].tag = MPI_MY_TAG;
      r[i].comm = MPI_COMM_WORLD;
      r[i].status = MPI_STATUS_IGNORE;
      threads[i] = myth_create(recv_func, (void *) (&r[i]));
      x_external += n_recv;
    }
  ...
  //printf("num_neighbors %d\n", num_neighbors);
    ...
    for (int i = 0; i < num_neighbors; i++)
    {
      ...
      s[i].buf = s_buffer;
      s[i].count = n_send;
      s[i].datatype = mpi_dtype;
      s[i].dest = neighbors[i];
      s[i].tag = MPI_MY_TAG;
      s[i].comm = MPI_COMM_WORLD;
      threads[i + num_neighbors] = myth_create(send_func, (void *) (&s[i]));
      s_buffer += n_send;
      }
